...
use App\Models\{Gejala, Penyakit, Rule, Riwayat, User};
...
class DiagnosaController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.diagnosa', compact('gejala', 'users'));
    }
    ...
    public function diagnosa(Request $request)
    {
        ...
        $username = auth()?->user()?->username ?? $request->username;
        $no_hp = auth()?->user()?->no_hp ?? $request->no_hp;
        $alamat = auth()?->user()?->alamat ?? $request->alamat;
        $jenis_kelamin = auth()?->user()?->jenis_kelamin ?? $request->jenis_kelamin;
        $umur = auth()?->user()?->umur ?? $request->umur;
        ...
        if(!auth()->check()) $route = 'riwayat';

        return redirect()->to(route($route, $riwayat->id));
    }
}
